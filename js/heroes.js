...
// ============================================
// Safety wrapper for getHeroesList - wait for gameDB to be ready
...
function getHeroShardsData() {
	if (!window.gameDB || !window.gameDB.Hero || !window.gameDB.Hero.Hero) {
		console.warn("Hero shard data not loaded yet, returning empty array");
		return [];
	}
	return window.gameDB.Hero.Hero["Hero Shards"] || [];
}
// Store hero-specific shard quantities
let heroSpecificShards = {};
// Load/save hero shards from localStorage
...
function handleHeroPetalClick(event, safeId, type, flowerId, clickedIndex, clickedValue, absoluteIndex) {
	...
	if (type === 'targ') {
		...
		if (clickedNumeric < currentNumeric) {
			...
			if (status) {
				const originalMessage = status.innerHTML;
				status.className = "status-pane status-error";
				status.innerHTML = `❌ Target level (${clickedValue}) cannot be lower than current level (${currentNumeric === 0 ? '0' : currentNumeric})`;
				setTimeout(() => {
					if (status.className === "status-pane status-error") {
						status.className = "status-pane";
						status.innerHTML = originalMessage;
					}
				}, 2000);
			}
			return;
...
// ============================================
// CREATE HERO CARD (with general shards checkbox)
// ============================================
function createHeroCard(hero, shardData) {
	...
	return `<div class="item-card" data-type="hero" data-name="${hero.name}" data-rarity="${hero.rarity}" data-generation="${hero.generation}" data-id="${safeId}">
        <div class="item-card-header">
            <img src="${getHeroImageFileName(hero.name)}" onerror="this.style.display='none';" style="height: 60px; width: 60px; object-fit: contain;">
...
// ============================================
// CALCULATIONS WITH HERO-SPECIFIC VS GENERAL SHARDS
// ============================================
...
function refreshCalculations() {
	...
	for (const card of cards) {
		...
		if (!from || !to) {
			...
			if (generalCb) generalCb.disabled = true;
			continue;
		}
		...
		if (generalCb) {
			if (hasEnoughHeroShards || !generalShardsAvailable) {
				...
				if (!generalShardsAvailable) {
					generalCb.parentElement.title = `No ${getGeneralShardType(hero.rarity).replace(/_/g, ' ')}s available in vault`;
				} else if (hasEnoughHeroShards) {
					generalCb.parentElement.title = `You already have enough ${hero.name} shards`;
				} else {
					generalCb.parentElement.title = "";
				}
			} else {
				generalCb.disabled = false;
				generalCb.parentElement.style.opacity = '1';
				// Add helpful tooltip showing achievable level
		...
		if (isLocked) {
			...
			for (const [res, amt] of Object.entries(costTotals)) {
				...
				if (res === `${hero.name}_shards`) {
					disp = `${hero.name} Shards`;
					// Calculate remaining hero shards after this upgrade
					const heroShardsLeft = availableHeroShards - amt;
					if (heroShardsLeft < 0) {
						costHtml += `<div class="resource-tag">📖 ${disp}: ${formatNumber(amt)} <span class="text-deficit">(${formatNumber(-heroShardsLeft)} short)</span></div>`;
					} else {
						costHtml += `<div class="resource-tag">📖 ${disp}: ${formatNumber(amt)} <span class="text-remaining">(${formatNumber(heroShardsLeft)} remaining)</span></div>`;
					}
					continue;
				}
		...
		if (costs.error) {
			// Calculate the highest achievable level with general shards
			const achievableLevel = getAchievableTargetLevel(hero, shardData, from, vault, otherLocked, availableHeroShards);
			const generalShardType = getGeneralShardType(hero.rarity);
			const availableGeneralShards = (vault[generalShardType] || 0) - (otherLocked[generalShardType] || 0);
			// Calculate needed shards for current target
			let neededForTarget = costs.heroShardsNeeded;
			let shortageForTarget = costs.heroShardsShortage;
			status.className = "status-pane status-error";
			status.innerHTML = `<strong>✗ INSUFFICIENT HERO SHARDS</strong><br>
        		Need ${neededForTarget} ${hero.name} shards, only have ${availableHeroShards}.<br>
        		<strong>💡 Tip:</strong> Check "General Shards" to use ${getGeneralShardType(hero.rarity).replace(/_/g, ' ')}s as fallback.<br>
        		<span style="color: #ff0000;">→ With your current ${availableGeneralShards} ${getGeneralShardType(hero.rarity).replace(/_/g, ' ')}s, you can reach up to level ${achievableLevel}.</span><br>
		...
		for (const [res, amt] of Object.entries(costTotals)) {
			if (res === `${hero.name}_shards`) {
				const remaining = availableHeroShards - amt;
				const disp = `${hero.name} Shards`;
				if (remaining < 0) {
					costHtml += `<div class="resource-tag">📖 ${disp}: ${formatNumber(amt)} <span class="text-deficit">(${formatNumber(-remaining)} short)</span></div>`;
				} else {
					costHtml += `<div class="resource-tag">📖 ${disp}: ${formatNumber(amt)} <span class="text-remaining">(${formatNumber(remaining)} remaining)</span></div>`;
				}
			} else {
		...
		if (!Object.keys(costTotals).length) costHtml = '<span>✨ No resources required</span>';
		const stepsInfo = stepsCount > 1 ? ` (${stepsCount} steps)` : '';
		const generalShardInfo = generalShardsUsed > 0 ? `<div class="resource-tag text-warning">⚠️ Using ${generalShardsUsed} general ${getGeneralShardType(hero.rarity).replace(/_/g, ' ')}s</div>` : '';
...
function onHeroGeneralShardsChange(safeId, isChecked) {
	...
	if (isChecked) {
		...
		const targetNumeric = parseFloat(to);
		// Always show notification when checking the box, even if target doesn't change
		const generalShardType = getGeneralShardType(hero.rarity);
		const availableGeneralShards = (vault[generalShardType] || 0) - (otherLocked[generalShardType] || 0);
		// Calculate needed shards for current target
		...
		if (achievableNumeric < targetNumeric) {
			...
			// Not enough general shards, but target wasn't reduced (shouldn't happen, but show warning)
			if (status) {
				status.className = "status-pane status-warning";
			...
			// Has enough general shards, show confirmation
			if (status) {
				status.className = "status-pane status-ok";

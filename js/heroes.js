...
function handleHeroPetalClick(event, safeId, type, flowerId, clickedIndex, clickedValue, absoluteIndex) {
    ...
    if (!heroFlowerStates[safeId]) {
        heroFlowerStates[safeId] = { currentMaxIdx: -1, targetMaxIdx: -1 };
    }
...
function updateHeroFlowerVisuals(safeId) {
    ...
    if (!card) return;
    const state = heroFlowerStates[safeId] || { currentMaxIdx: -1, targetMaxIdx: -1 };
...
function refreshCalculations() {
    ...
    for (const card of cards) {
        const hero = { name: card.dataset.name, rarity: card.dataset.rarity };
        ...
        if (!from || !to) {
            ...
            if (generalCb) { generalCb.checked = false; generalCb.disabled = true; }
            continue;
        }
        ...
        const { stepPoints, costTotals, stepsCount, heroShardsUsed, generalShardsUsed, heroShardsNeeded } = costs;
        let canAfford = true;

...
const RESOURCE_ITEMS = [
    {id:"bread",placeholder:"1.5M"},
    {id:"wood",placeholder:"2.3M"},
    {id:"stone",placeholder:"500K"},
    {id:"iron",placeholder:"125K"},
    {id:"gold",placeholder:"10K"},
    {id:"gems",placeholder:"10000"},
    {id:"truegold",placeholder:"500"},
    {id:"truegold_dust",placeholder:"1000"},
    {id:"tempered_truegold",placeholder:"25"},
    {id:"hero_xp",placeholder:"1000"},
    {id:"stamina",placeholder:"100"},
    {id:"master_manuscript",placeholder:"50"},
    {id:"general_emblem",placeholder:"20"},
    {id:"promotion_medallion",placeholder:"10"},
    {id:"nutrient_potion",placeholder:"20"},
    {id:"growth_manual",placeholder:"100"},
    {id:"advanced_taming_mark",placeholder:"10"},
    {id:"common_taming_mark",placeholder:"30"},
    {id:"pet_food",placeholder:"5000"},
    {id:"charm_design",placeholder:"80"},
    {id:"charm_guide",placeholder:"100"},
    {id:"artisans_vision",placeholder:"50"},
    {id:"gilded_threads",placeholder:"200"},
    {id:"satin",placeholder:"5000"},
    {id:"mithril",placeholder:"10"},
    {id:"forge_hammer",placeholder:"50"},
    {id:"mythic_general_shard",placeholder:"50"},
    {id:"epic_general_shard",placeholder:"200"},
    {id:"rare_general_shard",placeholder:"500"},
    {id:"building_speedup",placeholder:"2d 14h 35m"},
    {id:"research_speedup",placeholder:"5d 3h 20m"},
    {id:"training_speedup",placeholder:"1d 12h 5m"},
    {id:"master_speedup",placeholder:"1d"},
    {id:"general_speedup",placeholder:"10d 8h 45m"},
    {id:"mythic_gear",placeholder:"10"},
    {id:"hero_roulette_token",placeholder:"100"}
];
...
// ============================================
const SCORE_RULES = {
    truegold:2000, truegold_dust:1000, tempered_truegold:30000,
    forge_hammer:4000, widgets:8000, mithril:40000,
    advanced_taming_mark:15000, common_taming_mark:1150,
    general_emblem:6000, master_manuscript:60, speedup_min:30,
    roulette:8000, rare_general_shard:350, epic_general_shard:1220,
    mythic_general_shard:3040,
    troops:{1:1,2:2,3:3,4:5,5:7,6:11,7:16,8:23,9:30,10:39,11:49},
    gov_gear_score:36, gov_charm_score:70, pet_advancement_score:50
};
...
const DOMCache = {
    ...
    query: function(selector) {
        ...
        if (!this[key]) { this[key] = document.querySelector(selector); }
        return this[key];
    },
    queryAll: function(selector) {
        const key = 'qa_' + selector;
        if (!this[key]) { this[key] = document.querySelectorAll(selector); }
        return this[key];
    },
    clear: function() {
...
function buildResourceDisplay(costTotals, vault, lockedResources, heroName) {
    if (!costTotals || !Object.keys(costTotals).length) { return '<span>No resources required</span>'; }
    const filteredCosts = {};
    for (const [key, value] of Object.entries(costTotals)) {
        if (!key.startsWith('_')) { filteredCosts[key] = value; }
    }
    if (!Object.keys(filteredCosts).length) { return '<span>No resources required</span>'; }
    let basicResources = '', premiumResources = '', heroResources = '', speedupResources = '', otherResources = '';
    ...
    for (const [res, amt] of Object.entries(filteredCosts)) {
        ...
        if (isHeroResource(res)) {
            ...
            category = 'hero';
        } else if (basicKeys.includes(res)) { category = 'basic';
        } else if (premiumKeys.includes(res)) { category = 'premium';
        } else if (isSpeed) { category = 'speedup'; }
        ...
        if (category === 'hero') { heroResources += tag;
        } else if (category === 'basic') { basicResources += tag;
        } else if (category === 'premium') { premiumResources += tag;
        } else if (category === 'speedup') { speedupResources += tag;
        } else { otherResources += tag; }
    }
    let result = '';
...
function loadPointsFromData() {
    if (!gameDB.Points || !gameDB.Points.Points) { return; }
    ...
    for (const item of pointsData) {
        if (item.sg !== null && item.sg !== undefined) {
            ...
            if (typeof item.sg === 'string' && item.sg.includes(' per ')) {
                ...
                sgPoints[missionKey] = { type:'per_unit', points:parseFloat(parts[0]), per:parseFloat(parts[1]) };
            } else { sgPoints[missionKey] = parseFloat(item.sg); }
        }
    }
    const mappings = {
    ...
    for (const [key, rule] of Object.entries(mappings)) {
        if (sgPoints[key] !== undefined) { SCORE_RULES[rule] = sgPoints[key]; }
    }
    for (let i = 1; i <= 11; i++) {
        const key = `train_1_level_${i}_troop_s`;
        if (sgPoints[key] !== undefined) { SCORE_RULES.troops[i] = sgPoints[key]; }
    }
}
...
function showModal(options) {
    const { title, message, inputLabel, inputValue, confirmText, cancelText, onConfirm, onCancel, isDanger = false } = options;
...
document.addEventListener('keydown', function(event) {
    if (event.key === 'Escape') { closeNavMenu(); closeModal(); }
});
document.addEventListener('click', function(event) {
    const navLinks = DOMCache.get('navLinks');
    const hamburger = DOMCache.get('hamburgerIcon');
    if (navLinks && navLinks.classList.contains('show')) {
        if (!navLinks.contains(event.target) && !hamburger.contains(event.target)) { closeNavMenu(); }
    }
});
...
function togglePresetMenu() {
    ...
    if (!isOpen) {
        ...
        setTimeout(() => { document.addEventListener('click', closePresetMenuOutside); }, 10);
    } else {
...
async function createNewPreset() {
    ...
    try {
        const value = await showModal({
            ...
            inputLabel: 'Preset Name', inputValue: '',
        ...
        if (allPresets[trimmedValue]) {
            ...
            if (overwrite === undefined || overwrite === null) { return; }
...
function saveCurrentToPreset(presetName) {
    const existingPreset = allPresets[presetName] || { vault:{}, lockedUpgrades:[], selections:{}, heroFlowerStates:{}, heroSpecificShards:{}, heroWidgetQuantities:{} };
    const presetData = {
        vault:{ ...existingPreset.vault },
        lockedUpgrades:[...(existingPreset.lockedUpgrades || [])],
        selections:{ ...(existingPreset.selections || {}) },
        heroFlowerStates:{ ...(existingPreset.heroFlowerStates || {}) },
        heroSpecificShards:{ ...(existingPreset.heroSpecificShards || {}) },
        heroWidgetQuantities:{ ...(existingPreset.heroWidgetQuantities || {}) }
    };
    for (const item of RESOURCE_ITEMS) {
        const input = DOMCache.get(`vault_${item.id}`);
        if (input) { presetData.vault[item.id] = input.value; }
    }
    ...
    for (const [safeId, lockedData] of lockedUpgrades.entries()) {
        if (safeId.startsWith(page + '_')) {
            currentPageLockedUpgrades.push({ safeId, data: lockedData });
        }
    }
    ...
    document.querySelectorAll("select").forEach(select => {
        if (select.id) { presetData.selections[select.id] = select.value; }
    });
    document.querySelectorAll('input[type="checkbox"]').forEach(cb => {
        if (cb.id) { presetData.selections[cb.id] = cb.checked; }
    });
    document.querySelectorAll('input[type="number"], input[type="text"]').forEach(inp => {
        if (inp.id && !inp.id.includes('search')) { presetData.selections[inp.id] = inp.value; }
    });
    ...
    for (const id of buffInputs) {
        ...
        if (el) {
            if (el.type === 'checkbox') { presetData.selections[id] = el.checked;
            } else { presetData.selections[id] = el.value; }
        }
    }
    if (page === 'misc') {
...
function getCurrentPageInfo() {
    ...
    if (path.includes('forgehammer')) return { category:'forgehammer', display:'Forgehammer' };
    if (path.includes('hero-gear')) return { category:'herogear', display:'Hero Gear' };
    if (path.includes('war-academy')) return { category:'academy', display:'War Academy' };
    if (path.includes('buildings')) return { category:'building', display:'Building' };
    if (path.includes('widgets')) return { category:'widgets', display:'Widget' };
    if (path.includes('heroes')) return { category:'hero', display:'Hero' };
    if (path.includes('gov-gear')) return { category:'govgear', display:'GOV Gear' };
    if (path.includes('gov-charm')) return { category:'govcharm', display:'GOV Charm' };
    if (path.includes('pets')) return { category:'pet', display:'Pet' };
    if (path.includes('troops')) return { category:'troops', display:'Troops' };
    if (path.includes('misc')) return { category:'misc', display:'Misc' };
    if (path.includes('index')) return { category:'vault', display:'Vault' };
    return { category:'vault', display:'Vault' };
}
function getCurrentPageCategory() { return getCurrentPageInfo().category; }
function getCurrentPageDisplayName() { return getCurrentPageInfo().display; }
...
async function deletePreset(presetName) {
    ...
    try {
        ...
        localStorage.setItem("governor_presets", JSON.stringify(allPresets));
        if (currentPreset === presetName) { loadPreset("default"); }
        updatePresetSelect();
...
function loadPresetsFromStorage() {
    ...
    if (saved) {
        ...
        allPresets = { default: { vault:{}, lockedUpgrades:[], selections:{}, heroFlowerStates:{}, heroSpecificShards:{}, heroWidgetQuantities:{} } };
        localStorage.setItem("governor_presets", JSON.stringify(allPresets));
    }
...
window.clearAllSelections = function() {
    ...
    document.querySelectorAll('input[type="text"], input[type="number"]').forEach(input => {
        if (input.id && (input.id.includes('preset') || input.id.includes('search'))) { return; }
    ...
    document.querySelectorAll('select').forEach(select => {
        if (select.id && select.id.includes('preset')) { return; }
        select.selectedIndex = 0;
        select.dispatchEvent(new Event('change', { bubbles:true }));
    });
    document.querySelectorAll('input[type="checkbox"]').forEach(cb => {
        if (cb.id && cb.id.includes('preset')) { return; }
        cb.checked = false;
    ...
    for (const key of toRemove) { lockedUpgrades.delete(key); }
    if (category === 'building') {
        ...
        document.querySelectorAll('#globalBuildingBuffPercent, #globalPansMasterArtifact, #globalWolfPet, #globalKingPosition, #globalSaulsResourceful').forEach(el => {
            if (el) { el.value = '0'; el.dispatchEvent(new Event('change',{bubbles:true})); }
        });
    ...
    if (category === 'academy') {
        ...
        if (freshIdeasCb) { freshIdeasCb.checked = false; freshIdeasCb.dispatchEvent(new Event('change',{bubbles:true})); }
    }
    if (category === 'troops') {
    ...
    if (category === 'hero') {
        if (typeof heroFlowerStates !== 'undefined') {
            Object.keys(heroFlowerStates).forEach(key => {
                heroFlowerStates[key] = { currentMaxIdx:-1, targetMaxIdx:-1 };
            });
        ...
        if (typeof selectedMaxGeneration !== 'undefined') {
            ...
            if (genSelect) { genSelect.value = "7"; genSelect.dispatchEvent(new Event('change',{bubbles:true})); }
        }
    }
    if (category === 'widgets') {
    ...
    if (category === 'vault') {
        RESOURCE_ITEMS.forEach(item => { localStorage.removeItem(`vault_${item.id}`); });
    }
    const pageKey = getCurrentPageKey();
    if (pageKey) { localStorage.setItem(pageKey, '0'); }
    ...
    if (presetData) {
        if (presetData.selections) {
            ...
            for (const key of keysToRemove) { delete presetData.selections[key]; }
        }
        if (presetData.vault) {
            const vaultKeys = Object.keys(presetData.vault);
            for (const key of vaultKeys) { delete presetData.vault[key]; }
        }
...
function applyPresetToCurrentPage(preset) {
    ...
    if (preset.selections) {
        for (const [id, value] of Object.entries(preset.selections)) {
            ...
            if (element && element.tagName === "SELECT") {
                ...
                if (valueExists) { selectValues[id] = value; }
            }
        }
    }
    ...
    document.querySelectorAll("select").forEach(select => {
        if (select.id && !select.id.includes("presetSelect")) {
            if (select.id.startsWith('curr_')) { currSelects.push(select);
            } else if (select.id.startsWith('targ_')) { targSelects.push(select); }
        }
    });
    for (const select of targSelects) {
        if (selectValues[select.id] !== undefined) { select.value = selectValues[select.id]; }
    }
    ...
    if (preset.heroSpecificShards && typeof heroSpecificShards !== 'undefined') {
        ...
        document.querySelectorAll('.hero-shard-input[id^="hero_shard_"]').forEach(input => {
            ...
            if (heroSpecificShards[heroName] !== undefined) { input.value = heroSpecificShards[heroName]; }
        });
        if (typeof saveHeroShardsToStorage === 'function') { saveHeroShardsToStorage(); }
    }
    if (preset.heroWidgetQuantities && typeof heroWidgetQuantities !== 'undefined') {
        ...
        document.querySelectorAll('.hero-shard-input[id^="hero_widget_"]').forEach(input => {
            ...
            if (heroWidgetQuantities[heroName] !== undefined) { input.value = heroWidgetQuantities[heroName]; }
        });
        if (typeof saveHeroWidgetsToStorage === 'function') { saveHeroWidgetsToStorage(); }
    }
    const buffInputs = ['globalBuildingBuffPercent','globalPansMasterArtifact','globalWolfPet','globalKingPosition','globalSaulsResourceful','globalGroundWorksCheckbox','globalDoubleTimeCheckbox','globalTrainingBuffPercent','globalTrainingKingPosition','globalTrainingMobilizeCheckbox','globalTrainingKvKBonusCheckbox','globalResearchBuffPercent','globalResearchKingPosition','globalResearchFreshIdeasCheckbox','globalGatheringBuffPercent','globalMarchUnits','globalBisonGrip','globalBisonResource','globalBisonNode','heroRouletteCount','gatherBread','gatherWood','gatherStone','gatherIron'];
    for (const id of buffInputs) {
        if (preset.selections && preset.selections[id] !== undefined) {
            ...
            if (el) {
                ...
                } else { el.value = preset.selections[id]; }
            }
        }
    }
    document.querySelectorAll("select").forEach(select => {
    ...
    document.querySelectorAll('input[type="checkbox"]').forEach(cb => {
        if (cb.id) { cb.dispatchEvent(new Event('change', { bubbles:true })); }
    });
    document.querySelectorAll('input[type="number"], input[type="text"]').forEach(inp => {
    ...
    if (category === 'building' && typeof updateBuildingSpeedupBuffs === 'function') { updateBuildingSpeedupBuffs(); }
    if (category === 'academy' && typeof updateResearchSpeedupBuffs === 'function') { updateResearchSpeedupBuffs(); }
    if (category === 'troops' && typeof updateTrainingSpeedupBuffs === 'function') { updateTrainingSpeedupBuffs(); }
...
function secondsToSpeedupMinutes(seconds) { return Math.max(1, Math.ceil(seconds / 60)); }
...
function formatNumber(num) {
    if (num >= 1e9) { const val = num / 1e9; return val % 1 === 0 ? val.toFixed(0) + "B" : val.toFixed(2) + "B"; }
    if (num >= 1e6) { const val = num / 1e6; return val % 1 === 0 ? val.toFixed(0) + "M" : val.toFixed(2) + "M"; }
    if (num >= 1e3) { const val = num / 1e3; return val % 1 === 0 ? val.toFixed(0) + "K" : val.toFixed(2) + "K"; }
    return num.toLocaleString();
}
function getImageFileName(resourceId) {
    const imageMap = {
        bread:"Bread.webp", wood:"Wood.webp", stone:"Stone.webp", iron:"Iron.webp",
        gold:"Gold.webp", gems:"Gem.webp", truegold:"truegold.webp",
        truegold_dust:"truegold_dust.webp", tempered_truegold:"tempered_truegold.webp",
        hero_xp:"hero_xp.webp", stamina:"stamina.webp",
        master_manuscript:"master_manuscript.webp", general_emblem:"general_emblem.webp",
        promotion_medallion:"promotion_medallion.webp", nutrient_potion:"nutrient_potion.webp",
        growth_manual:"growth_manual.webp", advanced_taming_mark:"advanced_taming_mark.webp",
        common_taming_mark:"common_taming_mark.webp", pet_food:"pet_food.webp",
        charm_design:"charm_design.webp", charm_guide:"charm_guide.webp",
        artisans_vision:"artisans_vision.webp", gilded_threads:"gilded_threads.webp",
        satin:"satin.webp", mithril:"mithril.webp", forge_hammer:"forge_hammer.webp",
        ...
        building_speedup:"building_speedup.webp", research_speedup:"research_speedup.webp",
        training_speedup:"training_speedup.webp", master_speedup:"master_speedup.webp",
        general_speedup:"general_speedup.webp", mythic_gear:"mythic-gear.webp",
        hero_roulette_token:"hero_roulette_token.webp",
        bread_node:"bread_node.webp", wood_node:"wood_node.webp",
        stone_node:"stone_node.webp", iron_node:"iron_node.webp",
        gathering_speed:"gathering_speed.webp", grip_of_the_titan:"grip_of_the_titan.webp",
...
async function loadGameData(filesToLoad = null) {
    ...
    dataLoadPromise = (async () => {
        if (!filesToLoad) { filesToLoad = ["Points"]; }
...
function sanitizeNumericInput(input, allowEmpty = false) {
    ...
    if (isNaN(num) || num < 0) { input.value = ''; }
}
function attachInputValidation(input) {
    ...
    input.addEventListener('keydown', function(e) {
        if (e.key === '-' || e.key === 'Minus') { e.preventDefault(); return false; }
    });
...
function getResourceReductionPercentage() { return globalSaulsResourcefulPercent; }
...
function createGenericCard(config) {
    const { type, name, safeId, fromLevels, toLevels, highestLevel, imgUrl, currentSelectId, targetSelectId, onCurrentChange, onTargetChange, onUpgradeChange, onSpeedupChange, extraControls = '', customHeader = '', customBody = '', showSpeedup = false, showCheckbox = true } = config;
...
function genericRefreshCalculations(config) {
    const { type, getData, getLevels, getTargetLevels, calculateCosts, getBuffedTime = null, getDisplayName = null, getCategory = null } = config;
    let vault = getCurrentVault();
    const totalLocked = {};
    for (const [_, ld] of lockedUpgrades.entries()) {
        for (const [res, amt] of Object.entries(ld.costTotals)) {
            if (!res.startsWith('_')) { totalLocked[res] = (totalLocked[res] || 0) + amt; }
        }
    }
    let totalScore = 0;
    ...
    for (const card of cards) {
        ...
        if (!from || from === '' || !to || to === '') {
            ...
            if (activeCb) { activeCb.checked = false; activeCb.disabled = true; }
            if (speedCb) { speedCb.checked = false; speedCb.disabled = true; }
            continue;
        }
        ...
        if (isLocked) {
            ...
            const { stepPoints, costTotals, stepsCount, partialNote } = locked;
        ...
        const otherLocked = {};
        for (const [res, amt] of Object.entries(totalLocked)) {
            if (!res.startsWith('_')) { otherLocked[res] = amt; }
        }
        ...
        const { stepPoints, costTotals, stepsCount, partialNote } = costs;
        let canAfford = true;

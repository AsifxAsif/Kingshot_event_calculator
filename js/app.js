...
const RESOURCE_ITEMS = [
    ...
    { id: "hero_roulette", placeholder: "100" },
...
function buildResourceDisplay(costTotals, vault, lockedResources, heroName) {
    ...
    const premiumKeys = [
        ...
        'forge_hammer', 'widgets', 'mithril',
        'satin', 'gilded_threads', 'artisans_vision',
        'charm_guide', 'charm_design',
    ...
    for (const [res, amt] of Object.entries(filteredCosts)) {
        ...
        if (isHeroResource(res)) {
            ...
            const inventory = res.includes('shards') ?
                (typeof getHeroShardInventory === 'function' ? getHeroShardInventory(heroNameFromRes) : 0) :
                (typeof getHeroWidgetInventory === 'function' ? getHeroWidgetInventory(heroNameFromRes) : 0);
...
function loadPointsFromData() {
    ...
    for (const item of pointsData) {
        if (item.sg !== null && item.sg !== undefined) {
            const missionKey = item.mission.toLowerCase()
                .replace(/[^a-z0-9]/g, '_')
                .replace(/_+/g, '_');
...
function toggleNavMenu() {
    ...
    if (!overlay) {
        ...
        newOverlay.onclick = function () { closeNavMenu(); };
...
function saveCurrentToPreset(presetName) {
    ...
    // Save buff inputs
    const buffInputs = [
        'globalBuildingBuffPercent', 'globalPansMasterArtifact', 'globalWolfPet',
        'globalKingPosition', 'globalSaulsResourceful', 'globalGroundWorksCheckbox',
        'globalDoubleTimeCheckbox', 'globalTrainingBuffPercent', 'globalTrainingKingPosition',
        'globalTrainingMobilizeCheckbox', 'globalTrainingKvKBonusCheckbox',
        'globalResearchBuffPercent', 'globalResearchKingPosition', 'globalResearchFreshIdeasCheckbox'
    ];
    ...
    if (presetData) {
        ...
        if (presetData.lockedUpgrades) {
            presetData.lockedUpgrades = presetData.lockedUpgrades.filter(item => {
                return !item.safeId.startsWith(prefix) &&
                    !item.safeId.startsWith('promotion_') &&
                    !item.safeId.startsWith('troops_');
            });
        }
...
function applyPresetToCurrentPage(preset) {
    ...
    document.querySelectorAll("select").forEach(select => {
        if (select.id && !select.id.includes("presetSelect") &&
            !select.id.startsWith('curr_') && !select.id.startsWith('targ_')) {
            if (selectValues[select.id] !== undefined) {
                select.value = selectValues[select.id];
            }
        }
...
function attachInputValidation(input) {
    ...
    const isNumericField = input.id?.startsWith('vault_') ||
        input.id?.includes('qty') ||
        input.id?.includes('quantity') ||
        input.id?.includes('troop') ||
        input.placeholder?.toLowerCase().includes('quantity') ||
        input.classList.contains('hero-shard-input') ||
        input.type === 'number';
...
function genericRefreshCalculations(config) {
    ...
    for (const card of cards) {
        ...
        if (isLocked) {
            ...
            const timeHtml = getBuffedTime && locked.totalTimeSeconds ?
                `<div class="resource-tag">⏱️ Total Time: ${formatSecondsToTime(getBuffedTime(locked.totalTimeSeconds))}</div>` : '';
        ...
        const stepsInfo = stepsCount > 1 ? ` (${stepsCount} levels)` : '';
        const partialHtml = partialNote ? `<div class="resource-tag text-warning">${partialNote}</div>` : '';
        const timeHtml = getBuffedTime && costs.totalTimeSeconds ?
            `<div class="resource-tag">⏱️ Total Time: ${formatSecondsToTime(getBuffedTime(costs.totalTimeSeconds))}</div>` : '';

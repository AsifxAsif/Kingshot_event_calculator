...
function calculateAcademyCosts(dataArray, from, to, speedCheck, vault, otherLocked) {
    ...
    let stepPoints = 0, totalTimeSeconds = 0;
    const costTotals = {};
    ...
    let actualSpeedupUsed = 0, partialNote = '';
...
function refreshCalculations() {
    ...
    for (const card of cards) {
        ...
        const { stepPoints, costTotals, stepsCount, partialNote, totalTimeSeconds } = costs;
        let canAfford = true;
        ...
        const stepsInfo = stepsCount > 1 ? ` (${stepsCount} levels)` : '';
        const partialHtml = partialNote ? `<div class="resource-tag text-warning">${partialNote}</div>` : '';
        const timeHtml = totalTimeSeconds > 0 ?
            `<div class="resource-tag">⏱️ Total Time: ${formatSecondsToTime(getBuffedTime(totalTimeSeconds))}</div>` : '';

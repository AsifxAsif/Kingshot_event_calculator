...
function getTroopImageFileName(troopType) {
    const imageMap = { 'Infantry': 'Infantry.png', 'Cavalry': 'Cavalry.png', 'Archer': 'Archer.png' };
...
function refreshTroopsCalculations() {
    ...
    for (const card of cards) {
        ...
        displayTroopStatus(status, troopType, level, quantity, activeCb?.checked || false,
            speedCb?.checked || false, costTotals, stepPoints, totalTimeSeconds,
            canAfford, vault, displayOtherLocked);
    ...
    for (const card of promoCards) {
        ...
        if (speedCb) {
            ...
            const hasSpeedups = (vault[speedKey] || 0) > 0;
            const canUseSpeedup = canAfford && hasSpeedups && totalTimeSeconds > 0 &&
                fromLevel > 0 && toLevel > 0 && toLevel > fromLevel && quantity > 0;
        ...
        displayPromotionStatus(status, troopType, fromLevel, toLevel, quantity, activeCb?.checked || false,
            speedCb?.checked || false, costTotals, stepPoints, totalTimeSeconds,
            canAfford, vault, displayOtherLocked, pointsPerUnit);
    }
...
function loadTroops() {
    ...
    if (preset && preset.selections) {
        for (const [id, value] of Object.entries(preset.selections)) {
            if (id.startsWith('troop_lvl_') || id.startsWith('troop_qty_') ||
                id.startsWith('promo_from_') || id.startsWith('promo_to_') || id.startsWith('promo_qty_')) {
    ...
    setTimeout(() => { refreshTroopsCalculations(); }, 50);
}
